...
import { writingCombinationsMcqData } from '../data/writingCombinationsMcqData';
...
interface WritingCombinationsMcqScreenProps {
  navigateTo: (screen: Screen) => void;
  navigateBack: () => void;
}

const WritingCombinationsMcqScreen: React.FC<WritingCombinationsMcqScreenProps> = ({ navigateTo, navigateBack }) => {
    const [questions, setQuestions] = useState(() => shuffleArray(writingCombinationsMcqData).slice(0, 10));
    ...
    useEffect(() => {
        if (currentQuestion) {
            speak(currentQuestion.soundToSpeak);
        }
    }, [currentQuestion]);
    ...
    const handleEndQuiz = () => {
        saveTestResult({
            quizId: 'writing-combinations-mcq',
            quizTitle: 'املای دو‌گزینه‌ای ترکیب‌ها',
            score: score,
    ...
    const handleRestart = () => {
        setQuestions(shuffleArray(writingCombinationsMcqData).slice(0, 10));
    ...
    return (
        <div className="w-full max-w-2xl mx-auto p-4 md:p-8 animate-jump-in">
            <div className="text-center mb-6">
                <h2 className="text-2xl md:text-3xl font-bold text-brand-text dark:text-slate-100 mb-2">املای دو‌گزینه‌ای ترکیب‌ها</h2>
                <p className="text-lg text-slate-600 dark:text-slate-400">صدای زیر برای کدام ترکیب است؟ روی آن کلیک کن.</p>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800 p-6 md:p-10 rounded-3xl shadow-2xl">
                <button
                    onClick={() => speak(currentQuestion.soundToSpeak)}
                    className="w-full flex items-center justify-center gap-4 mb-8 bg-slate-100 dark:bg-slate-700 p-6 rounded-2xl cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"
                >
                    <SoundIcon className="w-10 h-10 text-brand-secondary"/>
...
export default WritingCombinationsMcqScreen;