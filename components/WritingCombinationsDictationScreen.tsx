...
import { writingCombinationsDictationData } from '../data/writingCombinationsDictationData';
...
interface WritingCombinationsDictationScreenProps {
  navigateTo: (screen: Screen) => void;
  navigateBack: () => void;
}

const WritingCombinationsDictationScreen: React.FC<WritingCombinationsDictationScreenProps> = ({ navigateTo, navigateBack }) => {
    const [questions, setQuestions] = useState(() => shuffleArray(writingCombinationsDictationData));
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (inputValue.trim() === currentQuestion.answer) {
            setIsCorrect(true);
            if (!isCorrect) { // only add score if it wasn't already correct
               setScore(s => s + 1);
            }
            speak('آفرین');
        } else {
    ...
    const handleEndQuiz = () => {
        saveTestResult({
            quizId: 'writing-combinations-dictation',
            quizTitle: 'املای ترکیب‌ها',
            score: score,
    ...
    const handleRestart = () => {
        setQuestions(shuffleArray(writingCombinationsDictationData));
    ...
    if (showResult) {
        return (
            <div className="flex flex-col items-center justify-center text-center p-8 animate-jump-in max-w-2xl mx-auto bg-white dark:bg-slate-800 rounded-3xl shadow-2xl">
                ...
                <p className="text-xl text-slate-600 dark:text-slate-400 mb-8">تو املای ترکیب‌ها رو تموم کردی.</p>
    ...
    return (
        <div className="w-full max-w-2xl mx-auto p-4 md:p-8 animate-jump-in">
            <div className="text-center mb-6">
                <h2 className="text-2xl md:text-3xl font-bold text-brand-text dark:text-slate-100 mb-2">املای ترکیب‌ها</h2>
                <p className="text-lg text-slate-600 dark:text-slate-400">به ترکیب‌های زیر گوش بده و در جای خالی بنویس.</p>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800 p-6 md:p-10 rounded-3xl shadow-2xl">
                ...
                <form onSubmit={handleSubmit} className="flex flex-col items-center gap-4">
                    <input
                        ...
                        className={`w-full max-w-xs text-center p-4 text-7xl font-bold border-4 rounded-2xl focus:ring-4 focus:border-transparent transition duration-300 dark:text-slate-100 dark:placeholder-slate-400 ${getInputClass()}`}
...
export default WritingCombinationsDictationScreen;