...
import { writingSignsDictationData } from '../data/writingSignsDictationData';
...
interface WritingSignsDictationScreenProps {
  navigateTo: (screen: Screen) => void;
  navigateBack: () => void;
}

const WritingSignsDictationScreen: React.FC<WritingSignsDictationScreenProps> = ({ navigateTo, navigateBack }) => {
    const [questions, setQuestions] = useState(() => shuffleArray(writingSignsDictationData));
    ...
    const handleEndQuiz = () => {
        saveTestResult({
            quizId: 'writing-signs-dictation',
            quizTitle: 'املای نشانه‌ها',
            score: score,
    ...
    const handleRestart = () => {
        setQuestions(shuffleArray(writingSignsDictationData));
    ...
    if (showResult) {
        return (
            <div className="flex flex-col items-center justify-center text-center p-8 animate-jump-in max-w-2xl mx-auto bg-white dark:bg-slate-800 rounded-3xl shadow-2xl">
                ...
                <p className="text-xl text-slate-600 dark:text-slate-400 mb-8">تو املای نشانه‌ها رو تموم کردی.</p>
    ...
    return (
        <div className="w-full max-w-2xl mx-auto p-4 md:p-8 animate-jump-in">
            <div className="text-center mb-6">
                <h2 className="text-2xl md:text-3xl font-bold text-brand-text dark:text-slate-100 mb-2">املای نشانه‌ها</h2>
                <p className="text-lg text-slate-600 dark:text-slate-400">صدای کدام نشانه را می‌شنوی؟ آن را در جای خالی بنویس.</p>
            </div>
...
export default WritingSignsDictationScreen;