...
import { Screen, WordQuizGroup } from '../types';
...
interface WritingWordsMcqQuizScreenProps {
  navigateTo: (screen: Screen) => void;
  navigateBack: () => void;
  quizGroup: WordQuizGroup;
}

const WritingWordsMcqQuizScreen: React.FC<WritingWordsMcqQuizScreenProps> = ({ navigateTo, navigateBack, quizGroup }) => {
    ...
    return (
        <div className="w-full max-w-2xl mx-auto p-4 md:p-8 animate-jump-in">
            <div className="text-center mb-6">
                <h2 className="text-2xl md:text-3xl font-bold text-brand-text dark:text-slate-100 mb-2">املای دو‌گزینه‌ای: {quizGroup.title}</h2>
                <p className="text-lg text-slate-600 dark:text-slate-400">کلمه‌ی درست را انتخاب کن.</p>
            </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    {answerOptions.map((answer) => (
                        <button
                            ...
                            disabled={isAnswered}
                            className={`relative w-full text-4xl font-bold p-4 rounded-2xl shadow-lg transition-all duration-300 flex justify-center items-center min-h-[120px] ${getButtonClass(answer)}`}
                        >
                            <span className="text-brand-primary dark:text-brand-primary-light">{answer}</span>
...
export default WritingWordsMcqQuizScreen;