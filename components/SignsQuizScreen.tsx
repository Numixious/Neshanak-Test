...
interface SignsQuizScreenProps {
...
const SignsQuizScreen: React.FC<SignsQuizScreenProps> = ({ navigateTo, navigateBack }) => {
    const [questions, setQuestions] = useState(() => shuffleArray(signsQuizData));
    ...
    const handleRestart = () => {
        setQuestions(shuffleArray(signsQuizData));
    ...
    return (
        <div className="w-full max-w-2xl mx-auto p-4 md:p-8 animate-jump-in">
            <div className="text-center mb-6">
                <h2 className="text-2xl md:text-3xl font-bold text-brand-text dark:text-slate-100 mb-2 cursor-pointer" onClick={() => speak("خودآزمایی نشانه‌ها")}>خودآزمایی نشانه‌ها</h2>
                <p className="text-lg text-slate-500 dark:text-slate-400 cursor-pointer" onClick={() => speak("صدای درست برای این نشانه کدام است؟")}>صدای درست برای این نشانه کدام است؟</p>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800 p-6 md:p-10 rounded-3xl shadow-2xl">
                <div className="flex justify-center items-center mb-8 cursor-pointer" onClick={() => speak(currentQuestion.letter)}>
                    <p className="text-9xl font-extrabold text-brand-primary dark:text-brand-primary-light">{currentQuestion.letter}</p>
                </div>
