...
import { wordReadingQuizData } from '../data/wordReadingQuizData';
import { Screen } from '../types';
import { speak } from '../services/speechService';
import { saveTestResult } from '../../services/quizService';

interface WordReadingQuizScreenProps {
...
const WordReadingQuizScreen: React.FC<WordReadingQuizScreenProps> = ({ navigateTo, navigateBack }) => {
    const [questions, setQuestions] = useState(() => shuffleArray(wordReadingQuizData));
    ...
    const handleEndQuiz = () => {
        saveTestResult({
            quizId: 'word-reading-quiz',
            quizTitle: 'خودآزمایی کلمه‌خوانی',
            score: score,
    ...
    const handleRestart = () => {
        setQuestions(shuffleArray(wordReadingQuizData));
    ...
    return (
        <div className="w-full max-w-2xl mx-auto p-4 md:p-8 animate-jump-in">
            <div className="text-center mb-6">
                <h2 className="text-2xl md:text-3xl font-bold text-brand-text dark:text-slate-100 mb-2 cursor-pointer" onClick={() => speak("خودآزمایی کلمه‌خوانی")}>خودآزمایی کلمه‌خوانی</h2>
                <p className="text-lg text-slate-500 dark:text-slate-400 cursor-pointer" onClick={() => speak("گزینه درست را انتخاب کن.")}>گزینه درست را انتخاب کن.</p>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800 p-6 md:p-10 rounded-3xl shadow-2xl">
                <div className="flex justify-center items-center mb-10 p-4 bg-slate-100 dark:bg-slate-700 rounded-2xl cursor-pointer" onClick={() => speak(currentQuestion.word)}>
                    <p className="text-6xl font-extrabold text-brand-text dark:text-slate-100">{currentQuestion.word}</p>
                </div>
...
export default WordReadingQuizScreen;